...
} from './services/CyberSessionService';
import type {
  CyberActionArgs,
  CyberActionId,
  CyberSessionSnapshot,
} from './services/CyberSessionService';
import { ExperimentService } from './services/ExperimentService';
import type { StoredExperiment } from './services/ExperimentService';
import {
  ScenarioLibraryService,
} from './services/ScenarioLibraryService';
import type {
  ScenarioSummary,
  ScenarioValidationReport,
} from './services/ScenarioLibraryService';
import { SecurityService } from './services/SecurityService';
import type { SecurityValidationReport } from './services/SecurityService';
...
import { createEmptyDocument, createScenarioDraft } from './StudioDocument';
import type { ReplayPlaybackState, StudioDocument } from './StudioDocument';

import type { BenchmarkReport } from './services/BenchmarkService';
...
export type RendererBackend = 'engine-gpu' | 'three-webgl';
...
export interface StudioSnapshot {
  ...
  cyberSimulationState: CyberSimulationState | null;
  cyberSimulationReplay: CyberSimulationReplay | null;
  selectedCyberScenarioId: string;
  hasCustomCyberScenario: boolean;
  isPlaying: boolean;
  isPaused: boolean;
  ...
  /**
   * Which renderer draws the viewport. Both consume the same engine scene
   * description and the same simulation state, so switching is a presentation
   * change rather than a different simulation.
   */
  ...
  /** Canonical cyber simulation session (seed, step mode, replay, errors). */
  cyberSession: CyberSessionSnapshot;
  /** Every action the engine exposes, with live availability. */
  cyberActionAvailability: Array<{
    id: CyberActionId;
    label: string;
    group: 'attack' | 'detection' | 'defense';
    description: string;
    available: boolean;
    reason: string | null;
  }>;
  /** Scenario library: engine catalog plus user authored scenarios. */
  scenarioLibrary: ScenarioSummary[];
  /** Structured telemetry captured for the session. */
  telemetryEvents: Array<import('@cyre/engine').TelemetryEvent>;
  ...
  securityReports: SecurityValidationReport[];
  /** Studio windowing state. */
  ...
  scenarioValidation: ScenarioValidationReport | null;
  replayPlayback: ReplayPlaybackState;
  inspectionFocusHostId: string | null;
...
export class StudioApplication {
  ...
  private selectedCyberScenarioId = 'lab-basic';
  private customCyberScenario: CyberScenarioDefinition | null = null;

  /** Owns the canonical CyberSimulation instance and its replay log. */
  readonly cyber = new CyberSessionService({
    onAction: (action, args, outcome) => this.recordCyberActionTelemetry(action, args, outcome),
    onChange: () => this.emit(),
  });

  /** Cyber scenario catalog plus user authored scenarios. */
  readonly scenarios = new ScenarioLibraryService();

  /** Structured telemetry for the active Studio session. */
  readonly telemetry = new TelemetryService('cyre-studio-session');

  /** Reproducible multi-seed experiment runner. */
  readonly experiments = new ExperimentService();

  /** Engine performance benchmarks. */
  readonly benchmarks = new BenchmarkService();

  /** Sandbox, policy and audit validation. */
  readonly security = new SecurityService();

  /** Studio windowing model. */
  readonly windows = new WindowManager({
    titleFor: (kind) => getWindowDefinition(kind).title,
  });

  /** Undo/redo for the scenario draft and the network graph. */
  readonly history = new HistoryStack<StudioDocument>(createEmptyDocument());

  private replayPlayback: ReplayPlaybackState = {
    replay: null,
    index: 0,
    playing: false,
    state: null,
  ...
  private inspectionFocusHostId: string | null = null;
  private activeThemeAppearance: 'dark' | 'light' = 'dark';
  private scenarioValidation: ScenarioValidationReport | null = null;
  ...
  private rendererBackend: RendererBackend = 'engine-gpu';
  ...
  /** Keeps the Studio snapshot in sync with the domain services. */
  private initializeStudioServices(): void {
    this.scenarios.subscribe(() => this.emit());
    this.telemetry.subscribe(() => this.emit());
    this.experiments.subscribe(() => this.emit());
    this.benchmarks.subscribe(() => this.emit());
    this.security.subscribe(() => this.emit());
    this.windows.subscribe(() => this.emit());
  ...
  /**
   * Selects a host from the live cyber simulation.
   *
   * This is deliberately not `selectNetworkNode`: the viewport renders the
   * cyber simulation, whose host ids (`web-server`, `admin-workstation`) are
   * not the network graph editor's node ids. Routing a viewport click through
   * `selectNetworkNode` would throw on every host and surface an error toast,
   * so the viewport has its own path into the same inspector.
   */
  selectCyberHost(hostId: string): void {
    try {
      const host = this.cyber.getState()?.hosts[hostId];
      if (!host) {
        this.editorShell.addNotification('error', `No such host: ${hostId}`);
        this.emit();
        return;
      }

      const properties: InspectorTarget['properties'] = [
        { key: 'id', label: 'ID', type: 'string', value: host.id },
        { key: 'name', label: 'Name', type: 'string', value: host.name },
        { key: 'type', label: 'Type', type: 'string', value: host.type },
        {
          key: 'accessLevel',
          label: 'Access level',
          type: 'string',
          value: host.accessLevel,
        },
        {
          key: 'compromised',
          label: 'Compromised',
          type: 'string',
          value: host.compromised ? 'yes' : 'no',
        },
        {
          key: 'isolated',
          label: 'Isolated',
          type: 'string',
          value: host.isolated ? 'yes' : 'no',
        },
        {
          key: 'services',
          label: 'Services',
          type: 'string',
          value: host.services.map((service) => service.name).join(', ') || 'none',
        },
        {
          key: 'vulnerabilities',
          label: 'Vulnerabilities',
          type: 'string',
          value: host.vulnerabilities.join(', ') || 'none',
        },
      ];

      this.inspector.selectTarget(host.id, host.name, properties);
      this.multiSelectionManager.select({ id: host.id, type: host.type, name: host.name });
      this.editorShell.setStatusMessage('Selected host: ' + host.name);
    } catch (error) {
      this.editorShell.addNotification(
        'error',
        'Select host failed: ' + this.errorMessage(error),
      );
    }
    this.emit();
  }
  ...
  addNetworkNodeFromPalette(itemId: string, x?: number, y?: number): void {
    try {
      ...
      this.commitNetworkDocument('Add ' + item.label);
  ...
  connectNetworkNodes(sourceId: string, targetId: string, edgeType?: string): void {
    try {
      ...
      this.commitNetworkDocument('Connect ' + sourceId + ' to ' + targetId);
  ...
  removeNetworkNode(nodeId: string): void {
    try {
      this.networkGraphEditor.removeNode(nodeId);
      this.commitNetworkDocument('Remove network node');
  ...
  removeNetworkEdge(edgeId: string): void {
    try {
      ...
      this.commitNetworkDocument('Remove network edge');
  ...
  renderScene(width: number, height: number, mode: string): void {
    try {
      ...
      const liveCyberState = this.cyber.getState();
      const scene = liveCyberState
        ? CyberWorldAdapter.toSceneGraph(liveCyberState)
        : (() => {
    ...
    try {
      const plugin = {
        ...
        async activate(): Promise<void> {
          // Optional base activation hook; Studio does not require setup.
        },
      };
  ...
  packageMobileGame(name: string): void {
    ...
    /** The renderer currently drawing the viewport. */
  getRendererBackend(): RendererBackend {
    return this.rendererBackend;
  }

  /**
   * Switches the viewport between the engine GPU renderer and the Three.js
   * WebGL renderer.
   *
   * Both read the same `CyberSimulationState` through the same scene graph, so
   * the switch is lossless: nothing about the simulation changes, only which
   * code turns it into pixels.
   */
  setRendererBackend(backend: string): void {
    if (backend !== 'engine-gpu' && backend !== 'three-webgl') {
      throw new Error('Invalid renderer backend: ' + backend);
    }

    if (backend === this.rendererBackend) {
      this.emit();
      return;
    }
    ...
    this.editorShell.addNotification(
      'success',
      'Renderer set to ' + (backend === 'engine-gpu' ? 'Engine GPU' : 'Three.js WebGL'),
    );
    this.emit();
  }
  ...
  exportSelectedCyberScenario(): string {
    const scenario = this.customCyberScenario ?? CYBER_SCENARIOS.find(
      (entry) => entry.id === this.selectedCyberScenarioId,
    );

    if (!scenario) {
      throw new Error(
        'Selected cyber scenario "' + this.selectedCyberScenarioId + '" does not exist.',
      );
    }
  ...
  importCyberScenario(json: string): void {
    ...
    // Imported documents join the library unless they shadow a catalog id,
    // in which case they stay an in-memory override of that scenario.
    if (!CYBER_SCENARIOS.some((entry) => entry.id === scenario.id)) {
      try {
        ...
        this.editorShell.addNotification(
          'warning',
          `Imported scenario kept for this session only: ${this.errorMessage(error)}`,
        );
      }
    }

    this.customCyberScenario = scenario;
    this.selectedCyberScenarioId = scenario.id;
    this.cyber.stop();
    this.editorShell.setStatusMessage(`Scenario imported: ${scenario.name}`);
    this.emit();
  }
  ...
  /** The full library: engine catalog plus user authored scenarios. */
  listScenarioLibrary(): ScenarioSummary[] {
    return this.scenarios.list();
  }

  /** Returns the definition currently selected for the simulation. */
  getSelectedScenario(): CyberScenarioDefinition | null {
    return this.customCyberScenario ?? this.scenarios.get(this.selectedCyberScenarioId);
  }

  /** Saves an authored scenario into the custom library and selects it. */
  saveCyberScenario(scenario: CyberScenarioDefinition): void {
    const saved = this.scenarios.save(scenario);
    this.customCyberScenario = null;
    this.selectedCyberScenarioId = saved.id;
    this.cyber.stop();
    this.editorShell.addNotification('success', `Scenario "${saved.name}" saved.`);
    this.emit();
  }

  removeCustomCyberScenario(scenarioId: string): void {
    this.scenarios.remove(scenarioId);
    if (this.selectedCyberScenarioId === scenarioId) {
      this.selectedCyberScenarioId = CYBER_SCENARIOS[0].id;
      this.customCyberScenario = null;
    }
    this.cyber.stop();
    this.editorShell.addNotification('info', `Scenario "${scenarioId}" removed.`);
    this.emit();
  }

  /**
   * Runs the engine security pipeline over the selected scenario: sandbox
   * execution, hostile payload rejection and the security audit system.
   */
  runSecurityValidation(): SecurityValidationReport {
    const scenario = this.getSelectedScenario();
    const json = scenario ? serializeCyberScenarioDefinition(scenario) : '';

    const report = this.security.validateScenarioJSON(
      scenario?.id ?? null,
      json,
      this.cyber.getState(),
    );
    ...
    this.editorShell.setStatusMessage(
      report.passed
        ...
        : 'Security validation reported issues.',
    );
    this.emit();
    return report;
  }

  validateCyberScenario(scenario: CyberScenarioDefinition): ScenarioValidationReport {
    const report = this.scenarios.validate(scenario);
    this.editorShell.setStatusMessage(
      report.isValid
        ? `Scenario "${scenario.id}" is valid (${report.issues.length} advisories).`
        : `Scenario "${scenario.id}" has ${report.issues.filter((issue) => issue.severity === 'error').length} errors.`,
    );
    this.emit();
    return report;
  }
  ...
  selectCyberScenario(scenarioId: string): void {
    ...
    if (!scenario) {
      throw new Error('Cyber scenario "' + scenarioId + '" does not exist.');
    }

    this.customCyberScenario = this.scenarios.isCustom(scenarioId) ? scenario : null;
    this.selectedCyberScenarioId = scenarioId;
    this.cyber.stop();
    this.editorShell.setStatusMessage(`Scenario loaded: ${scenario.name}`);
    this.emit();
  }

  /** Starts a simulation for the selected scenario without touching Play mode. */
  startCyberSimulationForSelectedScenario(seed?: number): void {
    const scenario = this.getSelectedScenario();
    if (!scenario) {
      throw new Error(
        'Selected cyber scenario "' + this.selectedCyberScenarioId + '" does not exist.',
    ...
    this.cyber.initialize(seed ?? scenario.seed, scenario);
    this.editorShell.setStatusMessage(
      `Simulation started for "${scenario.name}" (seed ${this.cyber.getSeed()}).`,
    );
    this.emit();
  }

  private ensureCyberSimulationForCurrentMission(): void {
    ...
    if (!this.cyber.isActive()) {
      this.cyber.initialize(scenario.seed, scenario);

      // The canonical laboratory opening only applies to the lab scenario.
      if (scenario.id === 'lab-basic') {
        this.cyber.runCanonicalOpening();
      }
    }
  }
  ...
  play(): void {
    try {
      ...
      this.ensureCyberSimulationForCurrentMission();
  ...
  stop(): void {
    try {
      ...
      this.cyber.stop();
      this.replayPlayback = { replay: null, index: 0, playing: false, state: null };
      this.reloadCurrentScenario();
  ...
  initializeCyberSimulation(seed = 42): void {
    this.cyber.initialize(seed);
    this.editorShell.setStatusMessage(`Cyber simulation initialized (seed ${seed}).`);
    this.emit();
  }

  executeCyberSimulationAction(
    method: string,
    args?: Record<string, unknown>,
  ): void {
    this.executeCyberAction(method as CyberActionId, {
      alertId: args?.alertId === undefined ? undefined : String(args.alertId),
      hostId: args?.hostId === undefined ? undefined : String(args.hostId),
  ...
  /**
   * Typed entry point for the attack / detection / defense actions used by
   * the Studio windows. Delegates straight to the engine simulation.
   */
  executeCyberAction(action: CyberActionId, args: CyberActionArgs = {}): void {
    try {
      this.cyber.execute(action, args);
      this.editorShell.setStatusMessage(`${describeCyberAction(action).label} completed.`);
    } catch (error) {
      this.editorShell.addNotification(
        'error',
        `${describeCyberAction(action).label} failed: ${this.errorMessage(error)}`,
      );
      throw error;
    }
  ...
  /** Advances the canonical simulation clock by one deterministic tick. */
  stepCyberSimulation(): number {
    try {
      const time = this.cyber.step();
      this.editorShell.setStatusMessage(`Stepped to t=${time} ms.`);
      this.emit();
      return time;
    } catch (error) {
      this.editorShell.addNotification('error', `Step failed: ${this.errorMessage(error)}`);
      throw error;
    }
  }

  setStepMode(enabled: boolean): void {
    this.cyber.setStepMode(enabled);
    this.editorShell.setStatusMessage(enabled ? 'Step mode enabled.' : 'Step mode disabled.');
    this.emit();
  }

  getCyberSimulationState(): CyberSimulationState | null {
    return this.cyber.getState();
  }

  getCyberSimulationReplay(): CyberSimulationReplay | null {
    return this.cyber.getReplay();
  }

  replayCyberSimulation(replay: CyberSimulationReplay): void {
    this.cyber.loadReplay(replay);
    this.editorShell.setStatusMessage('Cyber simulation replay loaded.');
    this.emit();
  }

  /** Serialises the current replay log to a downloadable JSON document. */
  exportCyberReplayJSON(): string {
    const replay = this.cyber.getReplay();
    if (!replay) throw new Error('No replay available. Start a simulation first.');
    return serializeCyberReplay(replay);
  }

  /** Loads a replay document from JSON into the session. */
  importCyberReplayJSON(json: string): void {
    const replay = deserializeCyberReplay(json);
    this.cyber.loadReplay(replay);
    this.editorShell.setStatusMessage(
      `Replay loaded: ${replay.actions.length} actions, seed ${replay.seed}.`,
    );
    this.emit();
  }

  /** Opens a replay for frame-by-frame playback without disturbing the session. */
  openReplayPlayback(replay: CyberSimulationReplay | null): void {
    if (!replay) {
      this.replayPlayback = { replay: null, index: 0, playing: false, state: null };
      this.emit();
      return;
    ...
    this.replayPlayback = {
      ...
      index: 0,
      playing: false,
      state: this.cyber.replayUpTo(replay, 0),
    };
    this.editorShell.setStatusMessage(
      `Replay ready: ${replay.actions.length} actions, seed ${replay.seed}.`,
    );
    this.emit();
  }

  /** Steps the replay playback window forward or backward. */
  seekReplayPlayback(index: number): void {
    const replay = this.replayPlayback.replay;
    if (!replay) throw new Error('No replay is open for playback.');

    const clamped = Math.max(0, Math.min(index, replay.actions.length));
    this.replayPlayback = {
      ...this.replayPlayback,
      index: clamped,
      state: this.cyber.replayUpTo(replay, clamped),
    };
    this.emit();
  }

  playReplayPlayback(): void {
    const replay = this.replayPlayback.replay;
    if (!replay) throw new Error('No replay is open for playback.');

    this.replayPlayback = { ...this.replayPlayback, playing: true };
    this.emit();
  }

  pauseReplayPlayback(): void {
    if (!this.replayPlayback.replay) return;
    this.replayPlayback = { ...this.replayPlayback, playing: false };
    this.emit();
  }

  getReplayPlayback(): ReplayPlaybackState {
    return {
      ...this.replayPlayback,
      replay: this.replayPlayback.replay,
      state: this.replayPlayback.state,
    };
  }

  private recordCyberActionTelemetry(
    action: CyberActionId,
    args: CyberActionArgs,
    outcome: { success: boolean; error: string | null; durationMs: number },
  ): void {
    const session = this.cyber.snapshot();
    const state = session.state;

    try {
      this.telemetry.recordAction(action, {
        ...
        attackerPosition: state?.attacker.position,
        evidenceViewed: (state?.evidence.length ?? 0) > 0,
        investigationPath: state?.defenderActions.map((entry) => entry.action),
      });
    } catch {
      // Telemetry must never break a running simulation.
    }

    if (args.hostId) {
      this.inspectionFocusHostId = args.hostId;
    }
  }

  acknowledgeMissionAlert(): void {    try {
      const runner = this.playModeController.getMissionRunner();
  ...
  // ---------------------------------------------------------------------
  // Document editing, undo/redo, appearance and project export
  // ---------------------------------------------------------------------

  /** Loads a scenario into the editable draft and records history. */
  setScenarioDraft(draft: CyberScenarioDefinition | null, label = 'Edit scenario draft'): void {
    const current = this.history.peek();
    this.history.commit(label, { ...current, scenarioDraft: draft });
    this.scenarioValidation = draft ? this.scenarios.validate(draft) : null;
    this.emit();
  }

  getScenarioDraft(): CyberScenarioDefinition | null {
    return this.history.peek().scenarioDraft;
  }

  /** Starts a fresh draft in the scenario editor window. */
  createScenarioDraft(id = 'custom-scenario'): void {
    this.setScenarioDraft(createScenarioDraft(id), 'Create scenario draft');
    this.editorShell.setStatusMessage('New scenario draft created.');
  }

  /** Commits the current draft into the scenario library and selects it. */
  commitScenarioDraft(): void {
    const draft = this.getScenarioDraft();
    if (!draft) throw new Error('There is no scenario draft to save.');

    const report = this.scenarios.validate(draft);
    const errors = report.issues.filter((issue) => issue.severity === 'error');
    if (errors.length > 0) {
      this.editorShell.addNotification(
        'error',
        `Scenario has ${errors.length} error(s): ${errors[0].message}`,
      );
      throw new Error(`Scenario validation failed: ${errors[0].message}`);
    }

    this.saveCyberScenario(draft);
  }

  validateScenarioDraft(): ScenarioValidationReport {
    const draft = this.getScenarioDraft();
    if (!draft) throw new Error('There is no scenario draft to validate.');

    const report = this.scenarios.validate(draft);
    this.scenarioValidation = report;
    this.emit();
    return report;
  }
  ...
  undo(): void {
    ...
    if (!restored) {
      this.editorShell.addNotification('info', 'Nothing left to undo.');
      this.emit();
      return;
    }

    this.applyDocument(restored);
    this.editorShell.setStatusMessage('Undo applied.');
    this.emit();
  }
  ...
  redo(): void {
    ...
    if (!restored) {
      this.editorShell.addNotification('info', 'Nothing left to redo.');
      this.emit();
      return;
    }

    this.applyDocument(restored);
    this.editorShell.setStatusMessage('Redo applied.');
    this.emit();
  }

  /** Re-applies a document snapshot to the live editors. */
  private applyDocument(document: StudioDocument): void {
    for (const node of this.networkGraphEditor.listNodes()) {
      this.networkGraphEditor.removeNode(node.id);
    }
    for (const node of document.networkNodes) {
      this.networkGraphEditor.addNode(node);
    }
    for (const edge of document.networkEdges) {
      this.networkGraphEditor.addEdge(edge);
    }

    this.scenarioValidation = document.scenarioDraft
      ? this.scenarios.validate(document.scenarioDraft)
      : null;
  }

  /** Records a network graph mutation so it can be undone. */
  private commitNetworkDocument(label: string): void {
    const current = this.history.peek();
    this.history.commit(label, {
      scenarioDraft: current.scenarioDraft,
      networkNodes: this.networkGraphEditor.listNodes(),
      networkEdges: this.networkGraphEditor.listEdges(),
    });
  }
  ...
  setAppearance(appearance: 'dark' | 'light'): void {
    this.activeThemeAppearance = appearance;
    this.editorShell.setStatusMessage(`${appearance === 'dark' ? 'Dark' : 'Light'} appearance active.`);
    this.emit();
  }

  toggleAppearance(): void {
    this.setAppearance(this.activeThemeAppearance === 'dark' ? 'light' : 'dark');
  }

  /** Serialises the whole project for the File ▸ Export Project action. */
    ...
    return JSON.stringify(
      {
        ...
        scenarioDraft: this.getScenarioDraft(),
        selectedCyberScenarioId: this.selectedCyberScenarioId,
        networkNodes: this.networkGraphEditor.listNodes(),
        networkEdges: this.networkGraphEditor.listEdges(),
        replay: this.cyber.getReplay(),
  ...
  private computeSnapshot(): StudioSnapshot {
    ...
    return {
      ...
      cyberSimulationState: this.cyber.getState(),
      cyberSimulationReplay: this.cyber.getReplay(),
      selectedCyberScenarioId: this.selectedCyberScenarioId,
      hasCustomCyberScenario: this.customCyberScenario !== null,
      ...
      cyberSession: this.cyber.snapshot(),
      cyberActionAvailability: CYBER_ACTIONS.map((action) => {
        const availability = evaluateActionAvailability(this.cyber.getState(), action.id, {
          alertId: this.cyber.getState()?.alerts[0]?.id,
          hostId: this.inspectionFocusHostId ?? undefined,
        });
      ...
      scenarioLibrary: this.scenarios.list(),
      telemetryEvents: this.telemetry.getEvents(),
      telemetrySessionId: this.telemetry.getSessionId(),
      experiments: this.experiments.list(),
      benchmarks: this.benchmarks.list(),
      securityReports: this.security.list(),
      windows: this.windows.list(),
      focusedWindowId: this.windows.getFocused()?.id ?? null,
      history: this.history.status(),
      scenarioDraft: this.history.peek().scenarioDraft,
      scenarioValidation: this.scenarioValidation,
      replayPlayback: this.getReplayPlayback(),
      inspectionFocusHostId: this.inspectionFocusHostId,
      appearance: this.activeThemeAppearance,
    };
  }
