export interface Transform25DOptions {
  x?: number;
  y?: number;
  z?: number;
  rotationX?: number;
...
export class Transform25D {
  ...
  constructor(options: Transform25DOptions = {}) {
    this.x = options.x ?? 0;
    this.y = options.y ?? 0;
    this.z = options.z ?? 0;
  ...
  validate(): void {
    ...
    for (const value of values) {
      if (!Number.isFinite(value)) {
        throw new Error('Transform25D values must be finite numbers.');
      }
    }
    if (this.scaleX === 0 || this.scaleY === 0 || this.scaleZ === 0) {
      throw new Error('Transform25D scale values cannot be zero.');
    }
  }

  clone(): Transform25D {
    return Transform25D.fromJSON(this.toJSON());
  }
  ...
  static fromJSON(data: Record<string, unknown>): Transform25D {
    return new Transform25D({
